...
class Battlezone{
 ...
 draw(){const c=this.cx;c.save();c.globalCompositeOperation="source-over";c.fillStyle="#000";c.fillRect(0,0,W,H);c.lineCap="round";
  ...
  const attractColors=[rgb.red,rgb.orange,rgb.green,rgb.blue,rgb.purple,rgb.lightOrange],attractColor=attractColors[Math.floor(this.attractColorFrame/30)%attractColors.length];
  ...
  for(const v of this.vectors){
   ...
   const alpha=Math.min(1,Math.max(.18,z/15)),scoreboard=asset==="highScore",color=this.colorized?(this.attractMode&&scoreboard?attractColor:(rgb[v[6]]||rgb.green)):(originalRed?rgb.red:rgb.green);
