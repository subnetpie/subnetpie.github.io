...
class Battlezone{
 ...
 draw(){const c=this.cx;c.save();c.globalCompositeOperation="source-over";c.fillStyle="#000";c.fillRect(0,0,W,H);c.lineCap="round";
  ...
  if(this.colorized){
   const tankPath=[17,16,12,13,14,20,18,15,14,17,16,19,21,17,15,16,19,18,20,21,3,0,4,7,6,2,3,7,11,10,6,5,9,10,13,9,8,11,12,8,4,5,1,2,1,0];
   ...
   for(const lines of groups.values()){
    ...
    const sum=new Map();
    // TNKOBJ's first TLABS establishes vertex 17 without drawing a segment.
    // Every following TVCTR/BVCTR targets one vertex in this exact source order.
    // The emitted AVG vector therefore ends at tankPath[i]; its start may be a
    // reposition/move and must not be assigned to the previous model edge.
    for(let i=0;i<Math.min(lines.length,tankPath.length-1);i++){const v=lines[i];if(!Number.isFinite(v[2]+v[3]))continue;const id=tankPath[i+1];let p=sum.get(id);if(!p){p=[0,0,0];sum.set(id,p)}p[0]+=v[2];p[1]+=v[3];p[2]++}
    if(lines.length){const v=lines[0];if(Number.isFinite(v[0]+v[1]))sum.set(tankPath[0],[v[0],v[1],1])}
