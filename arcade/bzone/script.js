...
class BzoneAudio{
 constructor(){this.ctx=null;this.node=null;this.reg=new Uint8Array(16);this.latch=0;this.phase=new Float64Array(4);this.enginePhase=0;this.noise=1;this.noisePhase=0;this.envShell=0;this.envExplosion=0}
 start(){if(!this.ctx){this.ctx=new (window.AudioContext||window.webkitAudioContext)({sampleRate:48000});this.node=this.ctx.createScriptProcessor(1024,0,1);this.node.onaudioprocess=e=>this.render(e.outputBuffer.getChannelData(0));this.node.connect(this.ctx.destination)}if(this.ctx.state==="suspended")this.ctx.resume()}
 ...
 control(d){this.start();d&=255;let old=this.latch;this.latch=d;if((d&4)&&!(old&4))this.envShell=1;if((d&1)&&!(old&1))this.envExplosion=1}
 render(out){const sr=this.ctx.sampleRate,enabled=(this.latch&32)!==0,motor=(this.latch&128)!==0,rev=(this.latch&16)!==0;for(let i=0;i<out.length;i++){let s=0;
  if(enabled){
...
if((y1<boundary&&y2>boundary)||(y2<boundary&&y1>boundary)){
  ...
  c.strokeStyle="rgba(80,255,80,"+alpha+")";c.lineWidth=1+z/12;c.beginPath();c.moveTo(x1,y1);c.lineTo(xb,boundary);c.stroke();
  c.strokeStyle="rgba(255,45,30,"+alpha+")";c.beginPath();c.moveTo(xb,boundary);c.lineTo(x2,y2);c.stroke();continue
}
